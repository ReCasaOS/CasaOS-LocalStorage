...
CURRENT_VERSION="$(${CURRENT_BIN_FILE} -v || ([ -x "${CURRENT_BIN_FILE_LEGACY}" ] && ${CURRENT_BIN_FILE_LEGACY} -v) || (stat "${CURRENT_BIN_FILE_LEGACY}" > /dev/null && echo LEGACY_WITHOUT_VERSION) || echo CURRENT_VERSION_NOT_FOUND)"
